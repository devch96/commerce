...
스파르타 MSA 트랙 Final 개별 프로젝트. 회원·상품·재고·주문·결제·쿠폰을 **독립 실행되는 마이크로서비스**로 나누고,
게이트웨이 JWT 인증, 동기 Saga 오케스트레이션(실 PG 결제창 연동), Transactional Outbox → Kafka 이벤트,
Redis 선착순 쿠폰, Elasticsearch 검색, ELK·Zipkin·Prometheus/Grafana 관측성까지 갖춘 커머스 백엔드입니다.
...
```
  ...
  Client ──────────────▶│  copa-gateway :8000  │  JWT 검증 → 서명 신원 토큰(X-Copa-Identity) 발급
      ...
      ┌────────────┬───────────────┼───────────────┬────────────┐
      ▼            ▼               ▼               ▼            ▼
 copa-user    copa-product     copa-order     copa-payment  copa-coupon
   :8081         :8082           :8085           :8084         :8086
 (회원·인증)  (상품·카테고리·  (동기 Saga       (토스·카카오    (쿠폰·선착순
              장바구니·검색)   오케스트레이터)   실 PG 연동)     FCFS)
...
- **서비스별 DB 분리**(MySQL 8.0 × 6) + 목적별 Redis 3대(인증 세션 / 상품 캐시 / 선착순 쿠폰).
...
```bash
# 1. 인프라 + 전체 서비스 (Docker)
docker compose up -d          # MySQL×6, Redis×3, Kafka, ELK, Zipkin, Prometheus/Grafana + 7개 서비스
...
├── copa-payment / copa-order / copa-coupon     # 각자 독립 Gradle 프로젝트 (멀티 모듈 아님)
...
| 캐시/원자연산 | Redis 7.2 ×3 (세션·캐시·선착순 Lua) |
